$(function(){
    ...
    //画网格
    //isCheck:是否检测当前手势位置在哪个网格
    function drawGrid(isCheck){
        context.save();
    ...
    //index,网格下标，从0开始
    //isCheck:是否检测当前手势位置在哪个网格
    function drawGridItem(index,isCheck){
        var x=(index%3*2+1)*(gridItemSize/2);
        var y=(parseInt(index/3)*2+1)*(gridItemSize/2);

        context.beginPath();
        context.arc(x,y,defaults.gridItemSize/2,0,Math.PI*2);
        if(isCheck){
            if(context.isPointInPath(drawState.x,drawState.y)){
                drawState.index=index+1;
            }
        }
        context.stroke();
        context.closePath();
